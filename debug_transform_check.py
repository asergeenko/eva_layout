...
def test_transform():
    ...
    interpretation2_bounds = (200, 300, 300, 400)  # offset = target position of bottom-left
    ...
    match1 = all(abs(actual_bounds[i] - interpretation1_bounds[i]) < tolerance for i in range(4))
    match2 = all(abs(actual_bounds[i] - interpretation2_bounds[i]) < tolerance for i in range(4))

    if match1:
        print("✅ Transform function works correctly! (Interpretation 1: offset = origin position)")
    elif match2:
        print("✅ Transform function works correctly! (Interpretation 2: offset = bottom-left position)")
    else:
...
def test_placed_carpet_creation():
    ...
    from carpet import PlacedCarpet, Carpet

    # Create simple carpet
    test_polygon = Polygon([(0, 0), (100, 0), (100, 100), (0, 100)])
    carpet = Carpet(polygon=test_polygon, filename="test.dxf")
    ...
    transformed_polygon = apply_placement_transform(test_polygon, x_offset, y_offset, angle)
    ...
    if placed_direct.polygon.bounds == test_polygon.bounds:
        print("❌ Problem found! PlacedCarpet.polygon contains original untransformed polygon")
    else:
