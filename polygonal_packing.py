...
class PolygonalCarpetPacker:
        ...
        for i, carpet in enumerate(sorted_carpets):
            if progress_callback:
                progress_callback(
                    ...
                    f"Полигональное размещение {i+1}/{total_carpets}",
                )
    ...
    def _sort_carpets_by_complexity(self, carpets):
        ...
        def complexity_score(carpet):
            ...
            num_vertices = (
                len(coords) - 1
            )  # -1 так как первая и последняя точки одинаковые

            # Convex hull ratio (насколько форма далека от выпуклой)
            convex_hull = polygon.convex_hull
            convex_ratio = (
                polygon.area / convex_hull.area if convex_hull.area > 0 else 0
            )
        ...
        for angle in rotation_angles:
            ...
            for position in valid_positions:
                ...
                if (
                    bounds[0] < 0
                    or bounds[1] < 0
                    or bounds[2] > self.sheet_width
                    or bounds[3] > self.sheet_height
                ):
                    continue
    ...
    def _calculate_no_fit_positions(
        ...
        for pos in positions:
            ...
            if (
                ...
                and y + poly_height <= self.sheet_height
                and x >= 0
                and y >= 0
            ):
    ...
    def _compute_nfp_positions(
        self, moving_polygon: Polygon, fixed_polygon: Polygon
    ) -> List[Tuple[float, float]]:
        """
        ...
        for coord in fixed_coords[
            :-1
        ]:  # -1 чтобы избежать дублирования последней точки
            fx, fy = coord
    ...
    def _compute_boundary_nfp_positions(
        self, polygon: Polygon
    ) -> List[Tuple[float, float]]:
    ...
    def _has_polygon_collisions(self, test_polygon: Polygon) -> bool:
        ...
        for placed_polygon in self.placed_polygons:
            if test_polygon.intersects(placed_polygon):
                ...
                if (
                    hasattr(intersection, "area") and intersection.area > 10.0
                ):  # Более щедрые допуски
                    return True
        return False

    def _evaluate_polygonal_placement(
        self, polygon: Polygon, position: Tuple[float, float]
    ) -> float:
...
def polygonal_bin_packing(carpets, sheet_size, verbose=True):
    ...
    if verbose and len(placed) > 0:
        ...
        logger.info(
            f"Полигональный алгоритм: размещено {len(placed)}, использование {usage_percent:.1f}%"
        )
