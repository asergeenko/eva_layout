...
def regenerate_kodiaq():
    ...
    if placed_layouts:
        ...
        print(
            f"First sheet: {len(first_sheet.placed_polygons)} carpets, {first_sheet.usage_percent:.1f}% usage"
        )
        ...
        for i, carpet in enumerate(first_sheet.placed_polygons):
            ...
            if abs(bounds[0]) < 10 and abs(bounds[1]) < 10:
                print(
                    f"⚠️  Found carpet at origin: {carpet.filename} - will be fixed by plot"
                )
