...
def analyze_coordinate_mismatch(dxf_path):
    ...
    print("🔍 АНАЛИЗ КООРДИНАТНОГО РАССОГЛАСОВАНИЯ")
    ...
    try:
        ...
        for layer in red_layers:
            ...
            splines = [
                elem for elem in elements_by_layer[layer] if elem["type"] == "SPLINE"
            ]
            ...
            if all_points:
                ...
                print(
                    f"   📊 SPLINE центр: ({spline_center_x:.1f}, {spline_center_y:.1f})"
                )
                print(
                    f"   📏 SPLINE размер: {spline_width:.1f} x {spline_height:.1f} мм"
                )

                # Находим соответствующий элемент в visualization
                print(
                    "   🎯 Этот элемент в visualization должен иметь примерно такие же координаты"
                )
...
def recreate_visualization_with_actual_coordinates():
    ...
    if len(polygons_for_packing) >= 2:
        placed_elements, unplaced_elements = bin_packing(
            polygons_for_packing, (200, 140)
        )
        ...
        for i, element in enumerate(placed_elements):
            ...
            print(
                f"      📍 Исходный центр полигона: ({orig_center_x:.1f}, {orig_center_y:.1f})"
            )
            print(
                f"      📍 Финальный центр полигона: ({final_center_x:.1f}, {final_center_y:.1f})"
            )
            ...
            for entity_data in original_data["original_entities"]:
                if (
                    entity_data["type"] == "SPLINE" and entity_data.get("color") == 1
                ):  # Только красные SPLINE
            ...
            if spline_polygon_points and len(spline_polygon_points) >= 3:
                try:
                    ...
                    if spline_polygon.is_valid:
                        ...
                        print(
                            f"      ✅ SPLINE полигон создан из {len(spline_polygon_points)} точек"
                        )
                    else:
                        print("      ❌ SPLINE полигон некорректный")
                except Exception as e:
                    print(f"      ❌ Ошибка создания SPLINE полигона: {e}")
            else:
                print("      ⚠️ Недостаточно SPLINE точек для создания полигона")
        ...
        ax.set_title(
            "ТОЧНАЯ ВИЗУАЛИЗАЦИЯ: Красные = SPLINE границы, Синий пунктир = polygon границы"
        )
...
def main():
    ...
    if dxf_success and viz_success:
        print("\n🎯 АНАЛИЗ ЗАВЕРШЕН!")
        print(
            "📊 Проверьте accurate_visualization.png - должна совпадать с autodesk.png"
        )
        print("🔧 Если они не совпадают, значит проблема в трансформации координат")
    else:
        print("\n❌ Ошибка в анализе")
