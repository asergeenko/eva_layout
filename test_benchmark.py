...
def run_benchmark():
    ...
    print(
        f"📄 Подготовлено: {available_sheets[0]['count']} листов {available_sheets[0]['width']}x{available_sheets[0]['height']} см"
    )
    ...
    sheet_area_mm2 = (available_sheets[0]["width"] * 10) * (
        available_sheets[0]["height"] * 10
    )
    ...
    print(
        f"🎯 Цель клиента: 2 листа ({(total_area_mm2/(2*sheet_area_mm2))*100:.1f}% загрузка)"
    )
