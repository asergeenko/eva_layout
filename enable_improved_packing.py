...
def check_current_setting():
    ...
    try:
        ...
        from layout_optimizer import (
            ...
            POLYGONAL_PACKING_AVAILABLE,
        )

        print("📊 ТЕКУЩИЕ НАСТРОЙКИ АЛГОРИТМОВ РАЗМЕЩЕНИЯ:")
        print("   Стандартный алгоритм: ✅ Всегда доступен")
        print(
            f"   Улучшенный алгоритм: {'✅ Да' if IMPROVED_PACKING_AVAILABLE else '❌ Нет'}"
        )
        print(
            f"   Полигональный алгоритм: {'✅ Да' if POLYGONAL_PACKING_AVAILABLE else '❌ Нет'}"
        )
        print()
        print(
            f"   Использовать улучшенный по умолчанию: {'✅ Да' if USE_IMPROVED_PACKING_BY_DEFAULT else '❌ Нет'}"
        )
        print(
            f"   Использовать полигональный по умолчанию: {'✅ Да' if USE_POLYGONAL_PACKING_BY_DEFAULT else '❌ Нет'}"
        )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Использование:")
        print(
            "  python enable_improved_packing.py status   - показать текущие настройки"
        )
        print(
            "  python enable_improved_packing.py enable   - включить улучшенный алгоритм"
        )
        print(
            "  python enable_improved_packing.py disable  - отключить улучшенный алгоритм"
        )
        print()
