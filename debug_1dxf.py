...
def analyze_1dxf():
    ...
    print(
        f"Carpet fits in sheet: width={width_mm < sheet_width_mm}, height={height_mm < sheet_height_mm}"
    )
    ...
    print(
        f"Rotated 90° fits: width={rot_width_mm < sheet_width_mm}, height={rot_height_mm < sheet_height_mm}"
    )
        ...
        for dxf_file in sorted(files):
            try:
                ...
                if polygon_data and polygon_data.get("combined_polygon"):
                    ...
                    print(
                        f"  {dxf_file.name}: {base_polygon.area:.0f} mm² area, bounds {base_polygon.bounds}"
                    )
    ...
    for i, layout in enumerate(placed_layouts, 1):
        ...
        for j, placed_carpet in enumerate(layout.placed_polygons):
            print(
                f"    {placed_carpet.filename}: pos=({placed_carpet.x_offset:.1f}, {placed_carpet.y_offset:.1f}), angle={placed_carpet.angle}°"
            )
    ...
    if len(placed_layouts) >= 1:
        ...
        if len(first_sheet.placed_polygons) == 1:
            ...
            print(
                f"Position: ({first_placed.x_offset:.1f}, {first_placed.y_offset:.1f})"
            )
            ...
            transformed = rotate(
                first_placed.polygon, first_placed.angle, origin="centroid"
            )
            transformed = translate(
                transformed, first_placed.x_offset, first_placed.y_offset
            )

            trans_bounds = transformed.bounds
            print(f"Transformed bounds: {trans_bounds}")
            print(
                f"Transformed size: {trans_bounds[2]-trans_bounds[0]:.1f} x {trans_bounds[3]-trans_bounds[1]:.1f} mm"
            )
            ...
            print(
                f"Remaining space after placing: {remaining_width:.1f} x {remaining_height:.1f} mm"
            )
            ...
            for i, other_carpet in enumerate(priority1_polygons[1:], 2):
                ...
                fits_normal = (
                    other_width <= remaining_width and other_height <= remaining_height
                )
                fits_rotated = (
                    other_height <= remaining_width and other_width <= remaining_height
                )

                print(
                    f"  {other_carpet.carpet_id} ({other_width:.1f}x{other_height:.1f}): normal={fits_normal}, rotated={fits_rotated}"
                )

                if fits_normal or fits_rotated:
                    print(
                        f"    ❗ {other_carpet.carpet_id} SHOULD FIT but didn't get placed!"
                    )
