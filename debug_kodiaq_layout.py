        ...
        try:
            ...
            if polygon_data and polygon_data.get("combined_polygon"):
                ...
                for copy_num in range(2, 6):  # _2, _3, _4, _5 copies
                    ...
                    print(
                        f"✅ Created carpet: {filename}, area: {base_polygon.area/10000:.1f} см²"
                    )
...
def test_kodiaq_density():
    ...
    total_carpet_area = sum(
        c.polygon.area
        for c in carpets
        if c.carpet_id not in [u.carpet_id for u in unplaced]
    )
    total_sheet_area = len(placed_layouts) * 140 * 200 * 100  # Convert to mm²
    utilization = (
        (total_carpet_area / total_sheet_area) * 100 if total_sheet_area > 0 else 0
    )
    ...
    if len(placed_layouts) > 0:
        ...
        for i, layout in enumerate(placed_layouts, 1):
            print(
                f"   Sheet {i}: {len(layout.placed_polygons)} carpets, {layout.usage_percent:.1f}% full"
            )

            # Analyze wasted space
            max_height = (
                max(c.polygon.bounds[3] for c in layout.placed_polygons)
            ...
            print(
                f"      Max height: {max_height:.0f}mm, wasted top space: {wasted_height:.0f}mm ({wasted_area/10000:.1f} см²)"
            )
