...
from layout_optimizer import rotate_polygon
...
def analyze_tetris_quality(carpet_polygon, angle=0):
    ...
    accessible_points = sum(
        1
        for p in test_points
        if not rotated.contains(
            Polygon([p, (p[0] + 1, p[1]), (p[0] + 1, p[1] + 1), (p[0], p[1] + 1)])
        )
    )
    ...
    return {
        ...
        "tetris_score": fill_ratio * 0.3
        + accessibility_ratio * 0.4
        + (top_space_height / 200) * 0.3,
        "bounds": bounds,
    }
...
def debug_carpet_8_orientations():
    ...
    try:
        ...
        for angle in [0, 90, 180, 270]:
            ...
            print(
                f"  Accessibility: {analysis['accessibility_ratio']:.3f} (space below accessible)"
            )
            print(
                f"  Top space: {analysis['top_space_height']:.0f}mm (free space above)"
            )
            print(f"  TETRIS SCORE: {analysis['tetris_score']:.3f}")
            print(
                f"  Bounds: ({analysis['bounds'][0]:.0f}, {analysis['bounds'][1]:.0f}) to ({analysis['bounds'][2]:.0f}, {analysis['bounds'][3]:.0f})"
            )
            print()
        ...
        print(
            f"  Improvement over worst: {((best['tetris_score'] - worst['tetris_score']) / worst['tetris_score'] * 100):.1f}%"
        )
        print()
