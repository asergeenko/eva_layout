...
class TestStreamlitIntegration:
    ...
    def setup(self):
        ...
        assert os.path.exists(self.sample_excel_path), "Файл sample_input.xlsx не найден"
    ...
    def test_full_streamlit_workflow_emulation(self):
        ...
        if df.shape[0] > 2:
            ...
            if df.shape[1] > 3:
                ...
                for idx, row in pending_orders.iterrows():
                    if pd.notna(row.iloc[3]):  # Проверяем что Артикул (column D) не пустой
        ...
        assert len(all_orders) == 37, f"Ожидалось 37 невыполненных заказов, найдено {len(all_orders)}"
        ...
        for i, order in enumerate(selected_orders):
            ...
            dxf_files.append((polygon, mock_file.name, mock_file.color, mock_file.order_id))
        ...
        scaled_polygons = scale_polygons_to_fit(dxf_files, reference_sheet_size, verbose=False)
        ...
        assert placement_rate >= 0.8, f"Размещено только {placement_rate*100:.1f}% заказов, ожидалось минимум 80%"
        ...
        assert sheets_used <= total_available_sheets, f"Использовано {sheets_used} листов, доступно {total_available_sheets}"
        ...
        for layout in placed_layouts:
            ...
            assert len(layout["placed_polygons"]) > 0, f"Лист {layout['sheet_number']} пустой"
    ...
    def test_excel_parsing_emulation(self):
        ...
        for order in all_orders:
            required_fields = ["sheet", "row_index", "article", "product", "order_id", "color", "product_type"]
            for field in required_fields:
                assert field in order, f"Поле {field} отсутствует в заказе {order.get('order_id', 'unknown')}"
            
            # Проверяем что артикул не пустой
            assert len(order["article"].strip()) > 0, f"Пустой артикул в заказе {order['order_id']}"
            
            # Проверяем цвет
            assert order["color"] in ["чёрный", "серый"], f"Некорректный цвет {order['color']} в заказе {order['order_id']}"
