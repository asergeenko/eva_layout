...
for i, layout in enumerate(placed_layouts[:5], 1):  # Первые 5 листов
    print(
        f"Лист {i} ({len(layout.placed_polygons)} ковров, {layout.usage_percent:.1f}% заполнение):"
    )
    ...
    for carpet in layout.placed_polygons:
        ...
        if not (at_left or at_right or at_bottom or at_top):
            center_count += 1
            print(
                f"  ⚠️  {carpet.filename.split('_')[0]}: x={min_x:.0f}-{max_x:.0f}, y={min_y:.0f}-{max_y:.0f} (В ЦЕНТРЕ!)"
            )
        elif at_left:
            print(
                f"  ✓ {carpet.filename.split('_')[0]}: x={min_x:.0f}-{max_x:.0f}, y={min_y:.0f}-{max_y:.0f} (прижат к левому краю)"
            )
    print()
...
print(
    f"  Прижаты к левому краю:  {left_edge_count} ({left_edge_count/total_carpets*100:.1f}%)"
)
print(
    f"  Прижаты к правому краю: {right_edge_count} ({right_edge_count/total_carpets*100:.1f}%)"
)
print(
    f"  Прижаты к нижнему краю: {bottom_edge_count} ({bottom_edge_count/total_carpets*100:.1f}%)"
)
print(
    f"  Прижаты к верхнему краю: {top_edge_count} ({top_edge_count/total_carpets*100:.1f}%)"
)
print(
    f"  В ЦЕНТРЕ (не прижаты):  {center_count} ({center_count/total_carpets*100:.1f}%)"
)
print()
...
if center_count == 0:
    ...
    print(
        f"⚠️  ВНИМАНИЕ! {center_count/total_carpets*100:.1f}% ковров не прижаты к краям"
    )
