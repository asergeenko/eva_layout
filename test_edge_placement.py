...
def visualize_layout(layout, sheet_width, sheet_height, filename):
    ...
    for carpet in layout.placed_polygons:
        ...
        ax.text(cx, cy, carpet.filename.split('_')[0],
               ha='center', va='center', fontsize=6, weight='bold')

    ax.set_xlim(-10, sheet_width + 10)
    ax.set_ylim(-10, sheet_height + 10)
    ax.set_aspect('equal')
    ax.set_title(f'Раскладка на листе {sheet_width} x {sheet_height} мм\n'
...
print(f"Сравните с tmp_test/bad_middle/*.png")
