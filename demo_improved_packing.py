...
def create_test_carpets():
    ...
    complex_shapes = [
        # L-образный коврик
        (
            [(0, 0), (250, 0), (250, 120), (120, 120), (120, 200), (0, 200)],
            "L-образный коврик",
        ),
    ...
    for i, (width, height, name) in enumerate(sizes):
        ...
        carpets.append(
            Carpet(polygon, f"{name}_{i+1}.dxf", "черный", f"order_{i+1}", 1)
        )
    ...
    for i, (coords, name) in enumerate(complex_shapes):
        ...
        carpets.append(
            Carpet(polygon, f"{name}_{i+1}.dxf", "черный", f"complex_{i+1}", 1)
        )
...
def demo_packing_comparison():
    ...
    print(
        f"Создано {len(carpets)} тестовых ковриков для размещения на листе {sheet_size[0]}x{sheet_size[1]} см\n"
    )
    ...
    placed_improved, unplaced_improved = improved_bin_packing(
        carpets, sheet_size, verbose=False
    )
    ...
    if placement_improvement > 0:
        print(
            f"🎯 Размещение: +{placement_improvement} ковриков ({(placement_improvement/len(carpets)*100):.1f}% улучшение)"
        )
    elif placement_improvement < 0:
        print(
            f"⚠️  Размещение: {placement_improvement} ковриков ({abs(placement_improvement/len(carpets)*100):.1f}% хуже)"
        )
    else:
    ...
    if utilization_improvement > 1:
        print(
            f"📈 Эффективность: +{utilization_improvement:.1f}% лучше использование листа"
        )
    elif utilization_improvement < -1:
        print(
            f"📉 Эффективность: {abs(utilization_improvement):.1f}% хуже использование листа"
        )
    else:
    ...
    if placement_improvement > 0 or utilization_improvement > 2:
        if time_ratio < 3:
            ...
            print(
                "💡 РЕКОМЕНДАЦИЯ: Улучшенный алгоритм лучше, но значительно медленнее."
            )
